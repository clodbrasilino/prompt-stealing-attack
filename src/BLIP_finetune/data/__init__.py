...
def create_dataset(dataset, config, min_scale=0.5):
    ...
    if dataset=='pretrain':
        ...
        train_dataset = LexicaDataset( return_text=config['return_text'], dataset_dir=config['image_root'], mode='train', input_transform=transform_train)
        val_dataset = LexicaDataset( return_text=config['return_text'], dataset_dir=config['image_root'], mode='test', input_transform=transform_test)
        test_dataset = LexicaDataset( return_text=config['return_text'], dataset_dir=config['image_root'], mode='test', input_transform=transform_test)
        return train_dataset, val_dataset, test_dataset
    else:
